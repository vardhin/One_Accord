...
- **Spell tables (330 spells + bare baseline)** — valid element-sigil spells only:
  modifiers never stand alone, unmodified multi-sigil strings are invalid, and
  repeats only tune stats. Scaffold generated by `generate_spells.py`, **every
  effect hand-authored** (no auto-fill).
...
- **Region tiers** — **Dense** (~7–8, returnable hubs) · **Mid** (Goodsprings-scale
  ...
  tiering: **Dense** — Vendur, Belgond, Rosetta, Teling, Digzarr · **Mid** — Mauzuli
  (Old Dock), Naerchu (Training Camp), Vengarz Hold, Velduri (Magizhee also mid) · **Mini** —
  Arghanzza, Vaelp (Sword Sanctum), Pathali (Divine Labyrinth), glyph hermit's hole,
  medicine hermit's homestead (5; the 3–4 target was soft). **Undecided** — Silent Core
  (may not be a walkable region at all; could dissolve into the endgame war).
  → `Regions/world-structure.md`
- **Magizhee** — the Goodsprings-scale starter settlement; the **hub** at the center
  of the human pocket (Old Dock north, Naerchu/Training Camp east, Vendur south,
  predator-mountains west). → `Regions/Magizhee/region-magizhee.md`
...
- **Belgond** — dense raw-materials supplier town west of Magizhee on the
  ...
  Sword Sanctum trail. → `Regions/Belgond/region-belgond.md`
- **Mauzuli** (the Old Dock) — **mid** region north of Magizhee with two coupled states:
  ...
  **Mauzuli** (echoing Machilipatnam / Masulipatnam). → `Regions/Mauzuli/region-mauzuli.md`
- **Teling** — **dense** mastery-smithing hub SE of Vendur (human-pocket side); the
  ...
  "Teling steel" (Telangana wootz). → `Regions/Teling/region-teling.md`
- **Velduri** — **mid** wealthy self-exiled **silver-elite** enclave (old-order
  ...
  *velli* (silver), quietly echoing Vendur. **South of Vendur** in the southern
  mountains; **mountain-defended, not a hillfort** (off-spine). → `Regions/Velduri/region-velduri.md`
- **Naerchu** (the Training Camp) — the coming-of-age / militia hardening settlement NE
  of Magizhee; the safe eastern early route. Labelled "the Training Camp" on the map by
  its function. → `Regions/Naerchu/region-naerchu.md`
- **Vaelp** (the Sword Sanctum) — mini, late-gated mountain sanctum west of the human
  pocket beyond Belgond; the true Soul Sword withdrew here and guards itself as a young
  sword maiden. The One Accord choice returns here after right-side lore lets the
  player reinterpret the trail. → `Regions/Vaelp/region-vaelp.md`
- **Vengarz Hold** — mid-size mobile wooden/tent/wagon stronghold (a camp they call a
  **Hold**), south of Rosetta, reached from Arghanzza via the puzzle route; remnant
  of an old anti-hive force from the hero's legion. → `Regions/Vengarz/region-vengarz.md`
- **Arghanzza** — mini post-ferry threshold across the water from Mauzuli; a
  mangrove-like possessed-matter pass and the **fork point of the hive side**. One path
  routes toward Vengarz Hold → Rosetta → Silent Core; the second path routes to Pathali.
  → `Regions/Arghanzza/region-arghanzza.md`
- **Rosetta ruins** — confirmed dense hazard/lore region; old research ground-zero
  where the hive first entered; north of Vengarz Hold, below the Silent Core.
  → `Regions/Rosetta/region-rosetta.md`
- **Digzarr** — dense great fallen city on the hive side: where the few-thousand era
  lived, where the early plague's first-generation horrors still reign, and the source
  of recovered pre-collapse engineering (Persian wheels, automations, crossbows,
  printing press) that feeds union upgrades. A raid/extraction region, not a settlement
  to unite. → `Regions/Digzarr/region-digzarr.md`
- **Pathali** (the Divine Labyrinth) — mini **maze-dungeon** off the **second**
  path out of Arghanzza (not the Vengarz route); channel-point of the forgotten
  deities. The **first Light-glyph** is earned by navigating/surviving the labyrinth.
  (Earlier "Ancient Divine Labyrinth"; before that "Ancient Deity Sanctum.")
  → `Regions/Pathali/region-pathali.md`
- **Medicine hermit's homestead** — **mini** small-household spoke on the **human
  ...
  near Velduri. Reward and naming deferred. → `Regions/MedicineHermit/region-medicine-hermit.md`
- **Glyph hermit's hole** — **mini** single-NPC spoke in the **SE of the hive side**,
  ...
  → `Regions/GlyphHermit/region-glyph-hermit.md`
- **Silent Core** — northernmost, most hive-concentrated ground. **Tier undecided:**
  ...
  endgame war structure (resolved in Stage 7 endgame design).
  → `Regions/SilentCore/region-silent-core.md`
...
- **Route shape** — Magizhee (hub) opens east → Naerchu (Training Camp) or south → Vendur,
  ...
  **forks**: puzzle route → Vengarz Hold → Rosetta → Silent Core, or a second path →
  Pathali (the Divine Labyrinth). West of Magizhee, the predator road leads by caravan to
  Belgond and then up to Vaelp (the Sword Sanctum). Region heat orders pressure; the map is anchored by natural
...
- **The Core Five** — the Magizhee starter NPCs (guard captain, healer,
