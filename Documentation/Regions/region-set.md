# The Region Set — Lore-Demanded Places

> The catalogue of regions the **lore actually requires** — not invented to fill a
> map, but the places the story already implies. Each entry says **why it must exist**,
> its **tier**, its **heat band**, and the **quest-hook** that sends the player there.
> Built on `../Lore/premise-and-the-one-accord.md` and
> `../Lore/the-deities-and-the-soul-economy.md`; placed on the spine in
> `connectivity-map.md`; tiers per `world-structure.md`.

---

## Design model: hybrid Fallout, stalemate framing

- **Scale = hybrid Fallout** (`OVERVIEW.md`): **FNV-scale dense hubs** (rich, returnable,
  multiple quests overlapping) + **Fallout-1-style tight mini spokes** (one problem, one
  reason to come). Target **~7–8 dense + 3–4 mini**.
- **Binding = FNV-loose** (decided): regions are **inhabited places**, not quest gates.
  Multiple quests thread through each; you can arrive before/without a quest. The
  log/agent-memory world (`../Mechanics/log-and-agent-memory.md`) wants this.
- **Framing = stalemate, not salvage.** Per the premise: you are **not the first
  explorer.** The lore sites are **already explored, looted, and done for** — and now
  *inhabited* by whoever stayed. The fantasy is **arriving late to a finished history**,
  not discovering virgin ruins.
- **Why each region exists** ties back to the soul economy: the hive (`Sauron`), the
  forgotten deities lending power through glyphs, the two soul swords, and the frozen war.
...
- **Why it exists:** the hero's old route to the **true Soul Sword.** Per revised canon
  ...
  itself** — now as a **young sword maiden** with recent grief, new terms (no demanded
  self-sacrifice), and **new trails up** beyond the hero's single path.
