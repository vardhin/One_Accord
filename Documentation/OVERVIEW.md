...
- Structure is Dark Souls–like: a handful of dense authored regions (target
  **~7–8 dense + 3–4 mini**) with quieter traversal corridors between them. Not a
  huge open world. **Three regions are concluded so far, and tiered: the starter
  settlement (mid), Arghanzza (mini), and Silvergate (dense — the sword-girl's
  birthplace, now a trade hub).** — `Regions/world-structure.md`
